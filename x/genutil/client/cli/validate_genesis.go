...
func ValidateGenesisCmd(mbm module.BasicManager, defaultNodeHome string) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			if len(genesisFilePath) > 0 {
				...
				if err = mbm.ValidateGenesisFrom(cdc, clientCtx.TxConfig, genesisFilePath); err != nil {
					return fmt.Errorf("error validating genesis from files: %w", err)
				}
	...
	cmd.Flags().String(flags.FlagGenesisFilePath, filepath.Join(defaultNodeHome, "genesis"), "the file path of app genesis states")
