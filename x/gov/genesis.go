...
func InitGenesisFrom(ctx sdk.Context, ak types.AccountKeeper, bk types.BankKeeper, k keeper.Keeper, importPath string) error {
...
func ExportGenesisTo(ctx sdk.Context, k keeper.Keeper, exportPath string) error {
	...
	bz, err := gs.Marshal()
	if err != nil {
		return fmt.Errorf("failed to marshal %s genesis state: %s", types.ModuleName, err)
	}
