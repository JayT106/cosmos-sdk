...
func PopulateValidatorQueuePendingFromIterator(
	...
	iter, err := store.Iterator(types.ValidatorQueueKey, storev2types.PrefixEndBytes(types.ValidatorQueueKey))
	if err != nil {
		return err
	}
	defer iter.Close()
...
func populateTimeQueuePendingFromIterator(
	...
	iter, err := store.Iterator(queueKey, storev2types.PrefixEndBytes(queueKey))
	if err != nil {
		return err
	}
	defer iter.Close()
